...
const Contract = () => {
  ...
  useEffect(() => {
    const fetchSignedContract = async () => {
      ...
      if (!error && contractData) {
        setSignedContract(contractData);
      }
    };
  ...
  const SignedContractView = ({ contract }: { contract: any }) => {
    if (!contract) return <div className="text-center p-8">טרם נחתם חוזה</div>;

    const teamColors = getTeamColors();

    return (
      <div 
        className="relative bg-white p-8 rounded-xl shadow-xl print:shadow-none print:p-0 border-4 mt-4"
        style={{
      ...
      >
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-center">חוזה חתום</h2>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <p className="font-semibold">מספר חולצה:</p>
              <p>{contract.jersey_number}</p>
            </div>
            <div>
              <p className="font-semibold">ערך חוזה:</p>
              <p>{contract.contract_value} יורו</p>
            </div>
          </div>
          
          <div className="mt-6">
            <p className="font-semibold">מחויבות מנטלית:</p>
            <p className="italic">"{contract.mental_commitment}"</p>
          </div>

          {contract.signature && (
            <div className="mt-6">
              <p className="font-semibold mb-2">חתימת השחקן:</p>
              <img src={contract.signature} alt="חתימת השחקן" className="h-16" />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <Tabs defaultValue="current" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="current">חוזה נוכחי</TabsTrigger>
            <TabsTrigger value="signed">חוזה חתום</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="signed">
            <SignedContractView contract={signedContract} />
          </TabsContent>
        </Tabs>
      </div>
    </div>
