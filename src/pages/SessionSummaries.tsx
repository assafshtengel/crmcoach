...
const SessionSummaries = () => {
  ...
  const summaryId = queryParams.get('id');
  const playerIdFromUrl = queryParams.get('player');
  ...
  const fetchSummaries = async () => {
    ...
    if (error) {
      ...
      toast('שגיאה בטעינת סיכומים');
      return;
    }
    setSummaries(data || []);

    // If there's a summary ID in the URL, open the dialog with that summary
    if (summaryId) {
      const summary = data?.find(s => s.id === summaryId);
      if (summary) {
  ...
  useEffect(() => {
    // If there's a player ID in the URL, set it as the selected player
    if (playerIdFromUrl) {
      setSelectedPlayer(playerIdFromUrl);
    }
    
    fetchSummaries();
  }, [selectedPlayer, summaryId, playerIdFromUrl]);

  const handleSummaryClick = (summary: SessionSummary) => {
    setSelectedSummary(summary);
    setOpenDialog(true);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        {summaries.length === 0 ? (
          <div className="bg-white/80 rounded-lg p-8 text-center">
            <p className="text-lg text-gray-600">אין סיכומי מפגשים זמינים.</p>
          </div>
        ) : (
                ...
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-2 mr-2">
                      ...
                      <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleSummaryClick(summary)}>
        ...
        {/* Dialog for showing selected summary */}
