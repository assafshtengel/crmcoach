...
const TaskChallenge: React.FC = () => {
  ...
  const { data: challenge, isLoading } = useQuery({
    queryKey: ['taskChallenge'], // שינינו את שם ה-query key
    queryFn: async () => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error("No user found");

      const { data: existingChallenge, error: fetchError } = await supabase
        .from('task_challenges') // שינינו את שם הטבלה
        .select('*')
        .eq('user_id', user.id)
        .maybeSingle();
  ...
  const updateChallenge = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['taskChallenge'] });
      toast({
