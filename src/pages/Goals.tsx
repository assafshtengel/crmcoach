...
const Goals = () => {
  ...
  const fetchGoals = async (uid: string) => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('goals').select('*').eq('user_id', uid).order('due_date', {
        ascending: true
      });
      if (error) {
        throw error;
  ...
  return <div className="container mx-auto py-6">
      ...
      <Tabs defaultValue="long-term" value={activeTab} onValueChange={value => setActiveTab(value as 'long-term' | 'short-term' | 'immediate')}>
        ...
        {['long-term', 'short-term', 'immediate'].map(type => <TabsContent key={type} value={type} className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <Progress value={getCompletionPercentage(type)} className="h-2 mx-[70px] px-[26px] text-lime-950" />
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4 mb-6 p-4 border rounded-lg bg-slate-50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input id="title" name="title" value={newGoal.title} onChange={handleInputChange} placeholder="הכנס כותרת למטרה" required />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="due_date">תאריך יעד</Label>
                      <Input id="due_date" name="due_date" type="date" value={newGoal.due_date ? new Date(newGoal.due_date).toISOString().split('T')[0] : ''} onChange={handleInputChange} required={type !== 'immediate'} />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="description">תיאור המטרה</Label>
                    <Textarea id="description" name="description" value={newGoal.description || ''} onChange={handleInputChange} placeholder="פרט את המטרה" className="min-h-[80px]" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="success_criteria">מדד הצלחה</Label>
                    <Input id="success_criteria" name="success_criteria" value={newGoal.success_criteria || ''} onChange={handleInputChange} placeholder="כיצד תדע שהשגת את המטרה?" />
                  </div>
                  <Button type="submit" className="w-full">הוספת מטרה</Button>
                </form>

                <div className="space-y-4">
                  {loading ? <p className="text-center py-4">טוען מטרות...</p> : getGoalsByType(type).length === 0 ? <p className="text-center py-4 text-gray-500">לא הוגדרו מטרות. הוסף את המטרה הראשונה שלך!</p> : getGoalsByType(type).map(goal => <Card key={goal.id} className={`border-2 ${goal.completed ? 'border-green-200 bg-green-50' : 'border-gray-200'}`}>
                        <CardHeader className="pb-2">
                          <div className="flex justify-between items-start">
                            <div className="flex items-center gap-2">
                              <Checkbox id={`completed-${goal.id}`} checked={goal.completed} onCheckedChange={() => toggleGoalCompletion(goal.id, goal.completed)} />
                              <CardTitle className={`text-lg ${goal.completed ? 'line-through text-gray-500' : ''}`}>
                                {goal.title}
                              </CardTitle>
                            </div>
                            <Button variant="ghost" size="sm" className="text-red-500 h-8 px-2" onClick={() => deleteGoal(goal.id)}>
                              מחיקה
                            </Button>
                          </div>
                          {goal.due_date && <CardDescription>
                              תאריך יעד: {new Date(goal.due_date).toLocaleDateString('he-IL')}
                            </CardDescription>}
                        </CardHeader>
                        <CardContent className="pt-0">
                          {goal.description && <p className="text-sm mb-2">{goal.description}</p>}
                          {goal.success_criteria && <div className="text-sm text-gray-600">
